...
object NimbusCheckBoxPainter extends ButtonPainterLike {
   ...
   def paint( state: State, c: Color, g: Graphics2D, x: Int, y: Int, width: Int, height: Int ) {
      if( state.isEnabled ) {
         val base       = ColorUtil.mixColorWithAlpha( NimbusDefaults.baseColor, c )
         val blueGrey   = NimbusDefaults.blueGreyColor
         if( state.isPressed ) {
            if( state.isFocused ) {
               if( state.isSelected ) {
         ...
         val base        = ColorUtil.mixColorWithAlpha( NimbusDefaults.baseColor, c2 )
         val blueGrey    = NimbusDefaults.blueGreyColor
         if( state.isSelected ) {
